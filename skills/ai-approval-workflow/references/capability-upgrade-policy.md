...
3. **Private split:** hostnames, tokens, SSH details, and business procedures stay in runtime config or private scripts.
...
6. **Confirmed:** the user has approved the gap, abstraction, files, and deployment plan.
...
- **Private adapter:** root-owned scripts, allowlist entries, host-specific config.
- **Workflow YAML:** schedule and composition only.

Do not put a private adapter into core just because one task needs it.
